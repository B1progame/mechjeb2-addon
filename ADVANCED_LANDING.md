...
Optional **Emergency survival divert: land if short, water if empty** distinguishes a target that is no longer affordable from an actually exhausted propulsion system. With some delta-v remaining but a negative landing fuel margin, it abandons the unreachable precision marker and locks the predicted dry-land impact or selects the nearest sampled dry terrain inside the remaining aerodynamic search radius. After a sustained zero-delta-v reading it instead targets the predicted water impact or the nearest reachable water for a survivable ditching. Aerodynamic guidance and airbrakes remain active, and final RCS correction is allowed inside 5 km altitude or 30 seconds to impact so the vehicle uses its remaining control authority at the end. It never activates on bodies without oceans or when fuel limits are deliberately ignored.
...
When Trajectories supplies `CorrectedDirection`, atmospheric guidance follows that complete planned attitude rather than blending it back toward retrograde. This keeps the flown angle of attack consistent with the drag/lift profile Trajectories simulated. A routine retrograde entry burn is inhibited whenever the target is ahead of the predicted impact because shortening an undershooting trajectory only makes the miss worse. Emergency targets are held internally after selection, diversion searches collapse with the remaining aerodynamic envelope instead of assuming a 10 km minimum, and no new shoreline target is accepted after powered landing commitment or below 2 km radar altitude.
...
The default **Atmospheric capture (no orbital boostback)** profile deliberately shifts the targeted-deorbit aim point forward along the orbital ground track—5 km past the selected landing site by default. The offset controls the deorbit-window timing, while the orbital maneuver itself remains a pure periapsis-lowering burn rather than adding a sideways target-capture plane change. Its default periapsis is halfway through the atmosphere (35 km on Kerbin), not the former wasteful −60 km underground trajectory. Because this shallow osculating orbit does not intersect the surface, its atmospheric guidance time is the next periapsis rather than an impossible surface-impact time. Orbital coast holds prograde and may wait through multiple passes for the rotating target to align with that periapsis; auto-warp slows near the configurable ground-track tolerance. Only then does the vehicle turn retrograde and commit the burn. Once Trajectories produces an aim-point prediction, the burn throttles down in stages and stops at the closest approach instead of letting a high-TWR engine jump through the target.

Above the atmosphere, only that deorbit burn and coast are allowed; target-capture and boostback burns are gated off. Powered lateral capture is additionally delayed until the vehicle is below 55% of the atmospheric height and has at least 250 Pa dynamic pressure by default. An unreachable target no longer starts a continuous high-altitude landing burn merely because its calculated capture time already exceeds the remaining flight time. After meaningful atmospheric entry, aerodynamic guidance and the powered accelerate/brake controller bend the predicted impact back toward the real target and remove horizontal velocity over its center. The thresholds persist through MechJeb's normal settings system and are configurable in the window.

Advanced Landing synchronizes an unchanged target to Trajectories only once. Repeatedly forcing `UpdateTrajectory` every two seconds reset its incremental predictor during long coasts and made the reported impact slide downrange. The orange overshoot marker, cyan landing target, aim error, actual/target periapsis, altitude, vertical speed, and exact powered throttle are all reported for validation.
